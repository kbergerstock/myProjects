...
cGAME_ENGINE::cGAME_ENGINE() : move_tree() , game()
{
    cout << " game engine constructed " << endl;
}
...
void cGAME_ENGINE::display_board(ostream& out)
{
    ...
    out << ( game.toMove() == WHITE ? "WHITE MOVES" : "BLACK_MOVES") << endl;
}
#endif
...
void cGAME_ENGINE::set_board(cFEN& fen)
{
    ...
    game.set_board(fen);
    // initilze the search tree
    move_tree.init(game);
}

//  calculate the next move from current position
void cGAME_ENGINE::uGo()
{
    // start the search
    move_tree.go();
}
